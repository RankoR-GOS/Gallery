...
import com.dot.gallery.feature_node.data.util.UriSerializer
...
/**
 * Serialized polymorphically wherever a [Media] is persisted or passed between processes, keyed by
 * the serial name of the concrete case. The names are pinned to the package this type used to live
 * in so that payloads written by earlier versions still decode; they are a storage format and must
 * not be updated to follow the class.
 */
@Serializable
@Parcelize
sealed class Media : Parcelable {
    ...
    @Serializable
    @SerialName("com.dot.gallery.feature_node.domain.model.Media.UriMedia")
    @Parcelize
    ...
    @Serializable
    @SerialName("com.dot.gallery.feature_node.domain.model.Media.ClassifiedMedia")
    @Parcelize
